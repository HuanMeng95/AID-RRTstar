...
function path = dubins_path(param,segments)
    %%%%%%%%%%%%%%%%%%%%%%%%% DEFINE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     curve_cut = sum(param.seg_param(1:3))/segments;
    ...
    if( types(2) == S_SEG )
        ...
        seg2 = [seg2(1,:) + mid_pt1(1) ;...
                seg2(2,:) + mid_pt1(2) ;...
                mod(zeros(1,length(t2)) + mid_pt1(3),2*pi)];
...
%   mid_pt3 =  dubins_segment( param3, mid_pt2,  types(3) , r);
%     if round(param3/curve_cut) ~=0
%         t3 = param3;
%         if( types(3) == L_SEG )
%             seg3 = [r* sin(mid_pt2(3)+t3) ; r* -cos(mid_pt2(3)+t3) ];
%             seg3 = [seg3(1,:) - r* sin(mid_pt2(3)) + mid_pt2(1) ;...
...
%     end
    ...
    path = [seg1, mid_pt1(1:3)', seg2, mid_pt2(1:3)', seg3];
end
...
function seg_end = dubins_segment(seg_param, seg_init, seg_type, r)
    ...
    if( seg_type == L_SEG ) 
        ...
        seg_end(3) = mod(seg_init(3) + seg_param,2*pi);
